...
class res_partner(models.Model):
    ...
    allow_consignment = fields.Boolean("SOC")
    # is_author = fields.Boolean("Autor", default=False)
    consignee_location_id = fields.Many2one('stock.location','Local de Consignação (SOC)')
    report_attachment_ids = fields.One2many('ir.attachment','consignment_partner_id',string='Relatórios de Consignação (SOC)')
    send_auto_email = fields.Boolean("Enviar Relatório Automático (SOC)")
    ...
    def create_consignee_location(self):
        ...
        if not self.env['stock.location'].search([('name','=', 'SOC'),('usage','=','internal')]):
        ...
        consignado_id = self.env['stock.location'].search([('name','=', 'SOC')]).id
        print(consignado_id)
        default_vals = location_obj.default_get(location_obj.fields_get())
    ...
    def write(self, vals):
        if vals.get('allow_consignment') is not None:
            ...
            if vals.get('allow_consignment'):
                ...
                raise UserError('Não foi possĩvel arquivar o Inventário SOC, possui produtos nele')
    ...
    def action_view_consignment_products(self):
        ...
        if len(estoque) > 0:
            ...
            raise Warning('Este cliente não possui produtos em SOC.')
    ...
    def create_xls_consignment_report(self):
        ...
        if len(estoque) > 0:            
            cabecalho = "Mapa de Consignação (SOC) Editora Hedra\ncomercial@hedra.com.br\n11-3097-8304\n\n"
            empresa = self.name + "\n"
